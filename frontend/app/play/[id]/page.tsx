...
export default async function Play({ params, searchParams }: { params: { id: string }, searchParams: { shareId: string } }) {
    ...
    return (
        <div className='relative w-full h-screen'>
            <PixiApp mapData={map_data} className='absolute w-full h-full' username={user.user_metadata.full_name} access_token={session.access_token} realmId={params.id} uid={user.id}/>
        </div>
    )
}